...
def accuracy(origset, proteinset):
    ...
    correcthresh = q3_single[len(statecount.keys())+1][correctcount] 
    correctcount = 100 * len(statecount.keys())
    for structure in structures2:
        correct = len(structure)
        if correct > correcthresh * (len(statecount.keys())+1):
            correctcount += correct
    correctcount += threshv(correcthresh)
