...
aunt(Person,NieceNephew):-married(NieceNephew,X),parent(Y,X),sibling(Person,Y),female(Person).
...
uncle(Person,NieceNephew):-married(NieceNephew,X),parent(Y,X),sibling(Person,Y),male(Person).
