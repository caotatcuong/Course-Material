...
//Neu co slot trong thì khoi too mot PCB moi voi processID chinh la ID cua slot ny
	pcb[ID] = new PCB(ID);
...
//GoiSetExitCode de dat exitcode cho tien trinh goi.
	pcb[pID]->SetExitCode(ec);
	pcb[pcb[pID]->parentID]->DecNumWait();
    
//Goi JoinRelease đe giai phong tien trinh cha dang đoi no(neu co) va ExitWait() de xin tien trinh cha cho phep thoat.
	pcb[pID]->JoinRelease();
	pcb[pID]->ExitWait();
